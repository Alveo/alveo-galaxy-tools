...
def add_arguments(parser):
  ...
  parser.add_argument(
      "--num_train_steps", type=int, default=100, help="Num steps to train.")
  ...
  #parser.add_argument("--best_metric_path", type=str, default="/home/ruiwang/Documents/medical letter/best_metric",
                      #help="")
  parser.add_argument("--out_model_info", type=str, default="/home/ruiwang/Documents/medical letter/model_info",
                      help="")
  ...
  parser.add_argument("--inference_input_file", type=str, default=None, #"\\tmp\\nmt_data_en_fr\\infer.txt",
  ...
  parser.add_argument("--inference_output_file", type=str, default=None, # default="\\tmp\\nmt_data_en_fr\\infer_trans.txt",
...
def extend_hparams(hparams):
  ...
  utils.print_out("  train_src=%s" % hparams.train_src)
  utils.print_out("  train_tgt=%s" % hparams.train_tgt)
  
  utils.print_out("  dev_src=%s" % hparams.dev_src)
  utils.print_out("  dev_tgt=%s" % hparams.dev_tgt)
  
  utils.print_out("  test_src=%s" % hparams.test_src)
  utils.print_out("  test_tgt=%s" % hparams.test_tgt)
  
  #utils.print_out("  out_hparams=%s" % hparams.out_hparam)
  ...
  # Source vocab
  src_vocab_file = hparams.vocab_src
  tgt_vocab_file = hparams.vocab_tgt
  src_vocab_size, src_vocab_file = vocab_utils.check_vocab_Alveo(
      src_vocab_file,
  ...
  if hparams.share_vocab:
    ...
    tgt_vocab_size, tgt_vocab_file = vocab_utils.check_vocab_Alveo(
        tgt_vocab_file,
        check_special_token=hparams.check_special_token,
        sos=hparams.sos,
        eos=hparams.eos,
  ...
  for metric in hparams.metrics:
    ...
    #best_metric_path = hparams.best_metric_path + "_best_" + metric
    #hparams.add_hparam("best_" + metric + "_dir", best_metric_path)
    hparams.add_hparam("best_" + metric, 0)  # larger is better
...
def create_hparams_Alveo(
    default_hparams, hparams_path, save_hparams=True):
  """Create hparams or load hparams from out_dir."""
...
def main(unused_argv):
  ...
  train_main(FLAGS, default_hparams, train_fn)

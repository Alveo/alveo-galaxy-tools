...
def run_internal_eval(
  ...
  dev_ppl = _internal_eval(loaded_eval_model, global_step, eval_sess,
                           ...
                           summary_writer, "dev")
  test_ppl = None
  if use_test_set and hparams.test_src and hparams.test_tgt:
    test_src_file = hparams.test_src
    test_tgt_file = hparams.test_tgt
...
'''
def run_sample_decode_Alveo(infer_model, infer_sess, hparams,
                      summary_writer, src_data, tgt_data):
  """Sample decode a random sentence from src_data."""
  with infer_model.graph.as_default():
    loaded_infer_model, global_step = model_helper.create_or_load_model_Alveo(
        infer_model.model, hparams.ckpt_path, infer_sess, "infer")
...
def run_internal_eval_Alveo(
    eval_model, eval_sess, hparams, summary_writer,
    use_test_set=True):
  """Compute internal evaluation (perplexity) for both dev / test."""
  with eval_model.graph.as_default():
    loaded_eval_model, global_step = model_helper.create_or_load_model_Alveo(
        eval_model.model, hparams.ckpt_path, eval_sess, "eval")
...
def run_external_eval_Alveo(infer_model, infer_sess, hparams,
                      summary_writer, save_best_dev=True, use_test_set=True):
...
def run_full_eval(model_dir, infer_model, infer_sess, eval_model, eval_sess,
                  hparams, summary_writer, sample_src_data, sample_tgt_data):
  """Wrapper for running sample_decode, internal_eval and external_eval."""
  run_sample_decode_Alveo(infer_model, infer_sess, hparams, summary_writer,
                    sample_src_data, sample_tgt_data)
  dev_ppl, test_ppl = run_internal_eval_Alveo(
      eval_model, eval_sess, hparams, summary_writer)
  dev_scores, test_scores, global_step = run_external_eval_Alveo(
...
def run_full_eval_Alveo(infer_model, infer_sess, eval_model, eval_sess,
                  hparams, summary_writer, sample_src_data, sample_tgt_data):
  """Wrapper for running sample_decode, internal_eval and external_eval."""
  run_sample_decode_Alveo(infer_model, infer_sess, hparams, summary_writer,
                    sample_src_data, sample_tgt_data)
